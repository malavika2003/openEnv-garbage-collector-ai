"""Medium grading: tighter fuel benchmark and overflow sensitivity."""
...
def grade(summary: EpisodeSummary) -> Dict[str, Any]:
    ...
    time_score = float(max(0.0, 1.0 - 0.45 * time_pressure))

    base = (
        0.48 * collection_ratio
        + 0.34 * fuel_score
        + 0.18 * time_score
    ) * 100.0

    penalty = summary.invalid_actions * 4.5 + summary.constraint_violations * 8.0
    penalty += min(summary.total_overflow_penalty_accumulated * 0.10, 20.0)
